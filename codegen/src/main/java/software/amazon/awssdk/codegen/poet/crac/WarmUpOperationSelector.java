...
public final class WarmUpOperationSelector {
     ...
     * call as a warm-up (e.g. every operation is streaming or deprecated).
     */
    ...
    /**
     * Preference order: verified simple method, accepts an empty request, fewest required input members, returns
     * output, read-only verb, then operation name as the deterministic tie-break.
     */
    ...
    private static int requiredInputMemberCount(OperationModel operation) {
        return (int) inputMembers(operation).stream().filter(MemberModel::isRequired).count();
    }
